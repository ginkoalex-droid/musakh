...
def _order_to_out(order: ReceivingOrder) -> ReceivingOrderOut:
    ...
    # Resolve cancelled_by name (loaded via relationship if available)
    cancelled_by_name = None
    if hasattr(order, 'cancelled_by_user_rel') and order.cancelled_by_user_rel:
        cancelled_by_name = order.cancelled_by_user_rel.name
